...
$("#TicketType").change(function () {
    ticketType = $("#ValidyPeriod").val();
});
$("#SchoolId").change(function () {
    schoolId = $("#SchoolId").val();

});
function getActivationCode(schoolId, ticketType) {
    debugger
    if (ticketType !== "" && schoolId !== "") {

        $.ajax({

            url: '/Tickets/GenerateTicket?SchoolId=' + schoolId + '&ValidyPeriod=' + ticketType,
            type: 'POST',

            async: false,

            data: { 'SchoolId': schoolId, 'ValidyPeriod': ticketType},
...
function saveAllGeneratedTickets() {
    ...
    $("#ticketTable TBODY TR").each(function () {
        var row = $(this);
        var ticket = {};
        ticket.SchoolId = row.find("TD").eq(0).html();
        ticket.ValidityPeriod = row.find("TD").eq(1).html();
        ticket.TicketNumber = row.find("TD").eq(2).html();
