...
function classResult(schoolId, schSessionId, termId, classId) {
    getMaxScore();//GetClassReport
    debugger
    $.ajax({
        url: '/School/GetClassReport?SchoolId=' + schoolId + '&SchSessionId=' + schSessionId + '&TermId=' + termId + '&ClassId=' + classId,
        type: 'POST',
        async: false,
        dataType:'json',
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                $.each(data[i], function (j, item) {
                    ...
                    for (var i = 0; i < item.Subject.length; i++) {
                        rows += '<tr>';
                        rows += "<td colspan=4 height='21' align='left'><font color='#000000'>" + item.Subject + "<br></font></td>";
